...
class LanguageParser(Parser):
    ...
    @_('ID ASSIGN variable PLUS variable')
    def command(self, p):
        generate_number(VariablesManager.get_location(p.ID), "a")
        print("ADD "+p.variable0+" "+p.variable1)
        generate_number(VariablesManager.get_location(p.ID, "a")
        print("STORE b a")

    # @_('ID ASSIGN ID PLUS NUMBER')
    # def command(self, p):
    #     generate_number(VariablesManager.get_location(p.ID1), "a")
    #     print("LOAD b a")
    #     generate_number(p.NUMBER, "c")
    #     print("ADD b c")
    #     generate_number(VariablesManager.get_location(p.ID0), "a")
    #     print("STORE b a")
    
    # @_('ID ASSIGN NUMBER PLUS ID')
    # def command(self, p):
    #     generate_number(VariablesManager.get_location(p.ID1), "a")
    #     print("LOAD b a")
    #     generate_number(p.NUMBER, "c")
    ...
    @_('ID ASSIGN NUMBER MULTI ID')
    def command(self, p):
        generate_number(p.NUMBER, "c")
        generate_number(VariablesManager.get_location(p.ID1), "a")
        print("LOAD b a")
        result_register = multiplication(['b', 'c'],['a', 'd', 'e'])
        generate_number(VariablesManager.get_location(p.ID0), "a")
        print("STORE "+result_register+" a")

    @_('ID ASSIGN ID MULTI NUMBER')
    def command(self, p):
        generate_number(VariablesManager.get_location(p.ID1), "a")
        print("LOAD b a")
        generate_number(p.NUMBER), "c")
    ...
    @_('ID ASSIGN ID DIV ID')
    def command(self, p):
        generate_number(VariablesManager.get_location(p.ID1), "a")
        print("LOAD b a")
        generate_number(VariablesManager.get_location(p.ID2), "c")
        print("LOAD d c")
        result_register = division(['b', 'd'],['a', 'c', 'e'])
        generate_number(VariablesManager.get_location(p.ID0), "a")
        print("STORE "+result_register+" a")

    @_('ID ASSIGN NUMBER DIV ID')
    def command(self, p):
        generate_number(p.NUMBER, "c")
        generate_number(VariablesManager.get_location(p.ID1), "a")
        print("LOAD b a")
        result_register = division(['b', 'c'],['a', 'd', 'e'])
        generate_number(VariablesManager.get_location(p.ID0), "a")
        print("STORE "+result_register+" a")

    @_('ID ASSIGN ID DIV NUMBER')
    def command(self, p):
        generate_number(VariablesManager.get_location(p.ID1), "a")
        print("LOAD b a")
        generate_number(p.NUMBER), "c")
    ...
    def command(self, p):
        ...
        generate_number(temp_location, "a")
        generate_number(p.VALUE, "b")
        print("STORE b a")
        print("PUT a")